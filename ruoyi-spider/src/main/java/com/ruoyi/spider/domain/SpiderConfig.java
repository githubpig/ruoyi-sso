...
public class SpiderConfig extends BaseEntity
{
    ...
    public SpiderConfig setEntryUrls(String entryUrls) {
        ...
        if(StringUtils.isNotEmpty(entryUrls)){
            ...
            if(arr!=null) {
                ...
                if(isURL(entryUrls)){
                    entryUrlsList.add(entryUrls);
                }
            }
        }
        if(CollectionUtils.isEmpty(this.getEntryUrlsList())){
            System.out.println("入口地址集合为空!!!!");
        }
        return this;
    }
    ...
    public SpiderConfig setSaveDb(Integer saveDb) {
        this.saveDb = saveDb;
        return this;
    }
